...
# This 1st projects takes me and my partner Chongshuang approximately 3 days, :D
# Below is my version, some codes of which might be slightly different from his.
...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while k <= num_rolls:
        ...
        k += 1
        if x == 1:
            count += 1 # Count times that dice() occurs；计算骰子结果为1的次数
    if count == 0:
        return sum
    else:
        return 1
...
def picky_piggy(score):
    ...
    """
    # BEGIN PROBLEM 2
    # "*** YOUR CODE HERE ***" I turned this line into 'comment' by adding '#', for there is a 'str' check in OK
    decimal_expansion = 142857
    if score % 6 != 0:
        digit = 6 - score % 6
    else:
        digit = 0
    result = decimal_expansion // pow(10, digit) % 10 
    if score != 0:
        return result
    else:
        return 7 
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls != 0:
        return roll_dice(num_rolls, dice) # 这里有点tricky，hhh
    else:
    ...
    def say(score0, score1):
        if who == 0:
            if score0 - last_score > running_high:
                print(max(score0 - last_score, running_high), "point(s)! That's a record gain for Player 0!")
            return announce_highest(who, score0, max(score0 - last_score, running_high))
        if who == 1:
            if score1 - last_score > running_high:
                print(max(score1 - last_score, running_high), "point(s)! That's a record gain for Player 1!")
            return announce_highest(who, score1, max(score1 - last_score, running_high))
    return say # 在不定义nonlocal的情况下保存信息 属实花了我不少功夫
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def averager(*arg): # average is the function that computes the average of ORIGINAL_FUNCTION called TRIALS_COUNT times
        n, sum = 0, 0 
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    n, max_score = 1, 0
    while n <= 10:
        count_average = make_averaged(roll_dice, trials_count)(n, dice)
        if count_average > max_score:
            max_rolls = n
            max_score = count_average
        n += 1
    return max_rolls
    # END PROBLEM 9
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    if picky_piggy(opponent_score) >= cutoff: # smaller num_rolls preferred
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    # I may finish this optional question if available.
